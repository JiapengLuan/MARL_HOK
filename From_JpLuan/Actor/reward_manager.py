...
class RewardManager():
    ...
    def calc_advantage(self, agent_id, hero_idx, final_reward, cur_value, next_value, all_hero_reward_detail):
        ...
        self.advantage[agent_id][hero_idx] = self.advantage[agent_id][hero_idx] * self._gamma * self._lamda + delta
        reward_sum = self.advantage[agent_id][hero_idx] + cur_value

        LOG.info("hero_idx:%d final_reward:%f advantage:%f reward_sum:%f" % 
                 (hero_idx, final_reward, self.advantage[agent_id][hero_idx], reward_sum))
    ...
    def calc_final_reward(self, hero_idx, all_hero_reward_detail):
        ...
        for key in all_hero_reward_detail[hero_idx]:
            ...
            final_reward += value
            LOG.info("hero_idx:%d reward_name:%s reward_value:%f" % (hero_idx, key, value))

        LOG.info("hero_idx:%d my_final_reward:%f" % (hero_idx, final_reward))
